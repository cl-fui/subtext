...
(defun demo ()
  (let ((so *standard-output*)
	...
	(terpri stream)
	(prin stream "Welcome to " ("bg-greenish" "SubText™") #\. #\newline #\newline
	      "SubText is an initiative to create a Lispy, mostly-text-based user interface.
...
Click on "
	      ((make-instance 'button :code (lambda () (news))) "N E W S  D E M O")
...
Start a SWANK server on port 5000 and try our "
	      ((make-instance 'button :code (lambda () (repl))) "REPL Lisp Envronment")
	      " - type some Lisp commands or some nonsense to invoke the SubText debugger..


"
	      ("small" "¹ Feel free to use C-x C-c to quit..." ))
	(finish-output stream))
      )))
